...
Although Texas congressional districts are the ones being assessed for Section 2 Voting Rights Act violations in this case study, the analysis here is based on statewide elections. This is a common practice in redistricting analysis; the problem with including congressional elections in the election inventory is that it is unclear how votes for one congressional candidate would transfer to votes for a different candidate [@BeckerDuchinGoldHirsch2021]. By leveraging statewide elections instead of local elections, "apples-to-apples" comparisons across different regions of the state are able to be made; after all, the same set of candidates competed everywhere in the state [@BeckerDuchinGoldHirsch2021].Note that when performing $\S$2 Voting Rights Act analysis, it is optimal to include data from the past ten years [@BeckerDuchinGoldHirsch2021]. However, it is not always possible to meet this ideal  due to data availability and precinct instability [@BeckerDuchinGoldHirsch2021]. Here, only statewide election data from 2024 is included due to time and compute power constraints.


Note that the election data is leveraged to determine whether or not the minority-preferred candidates are elected [@BeckerDuchinGoldHirsch2021]. In order to accomplish this, the primary (and primary runoff, if applicable) for a specific office in a given year is linked to the general election for that same office and year [@BeckerDuchinGoldHirsch2021]. Success is measured by the minority-preferred candidate suceeding at all stages of the electoral process [@BeckerDuchinGoldHirsch2021].

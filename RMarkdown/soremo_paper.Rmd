...
For the converse, Axiom 2 forces $T(\pi) = \sum_d t(d, \pi)$. The hypothesis that
$t(d, \pi)$ depends on $\pi$ only through $p^g_d(\pi)$ writes
$t(d, \pi) = \phi(p^g_d(\pi))$ for some $\phi$. Axiom 3 (Monotonicity) requires
$\phi$ weakly increasing; Axiom 4 (Normalization) at the boundary cases requires
...
### 4.3.4 The Neutral Ensemble: ReCom with VRA-Relevant Constraints {#sec:neutral-ensemble}

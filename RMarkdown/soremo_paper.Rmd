...
We adopt a Bayesian multinomial--Dirichlet model:
...
$$
V_p \sim \text{Multinomial}(N_p, \theta_p).
$$
. Posterior samples are used to compute group preferences, district-level win probabilities, and uncertainty distributions.
...
calibrated probabilities produced in the ecological inference layer.
...
The $R \times C$ model of Section 4.2 is fit to each contest in Table 2 and report posterior support estimates, preference identifications, and per-precinct count posteriors at the octile resolution.

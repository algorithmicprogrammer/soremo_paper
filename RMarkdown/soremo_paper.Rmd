...
**Proposition.** The functional $O_g(\pi) = \sum_d p^g_d(\pi)$ satisfies A1–A4. Moreover, any functional satisfying A1–A4 with district contribution $t(d, \pi)$ depending on $\pi$ only through $p^g_d(\pi)$ takes the form $T(\pi) = \sum_d \phi(p^g_d(\pi))$ for some weakly increasing $\phi: [0,1]
...
The p-value is $$p = \frac{|\{j : T(Y^{(j)}) \leq T(\pi_0)\}| + 1}{M + 1}.$$
